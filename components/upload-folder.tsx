...
interface Provider {
  ...
  speed?: string;
  supportedTypes: string[]; // Tipos de archivos soportados: "image", "video", "all"
  description?: string; // Descripción adicional del proveedor
}
...
// Actualizar la lista de proveedores disponibles con información de tipos soportados
...
//     description: "Optimizado para imágenes y videos",
//   },
//   {
//     id: "gdrive",
//     name: "Google Drive",
...
//     supportedTypes: ["all"],
//     description: "Almacenamiento general",
//   },
//   {
//     id: "s3",
//     name: "Amazon S3",
...
//     description: "Especializado en imágenes",
//   },
//   {
//     id: "vimeo",
//     name: "Vimeo",
...
//     description: "Especializado en videos",
//   },
// ];
...
export default function UploadFolder() {
  ...
  // const [isUploading, setIsUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  // const [selectedProviders, setSelectedProviders] = useState<StorageProvider[]>(
  //   []
  // ); // Por defecto Cloudinary seleccionado
  const [providerProgress, setProviderProgress] = useState<
    Record<string, Record<string, number>>
  >({});
  const [providerSpeeds, setProviderSpeeds] = useState<
  ...
  const { availableProviders, toggleProviderSelection, selectedProviders } = useBoundStore(
    (s) => s
  );
  ...
  //       if (prev.some((prov) => prov._id === provider._id)) {
  //         return prev.filter((prov) => prov._id !== provider._id);
  //       } else {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all(
        uploadFiles.map(async (u) => {
          await uploadFile(u, folderName, selectedProviders);
        })
      );
    } catch (error) {
      console.log(error);
    ...
    //   try {
    //     console.log("procesando", file, "total", uploadFiles.length)
  ...
  const videoCount = uploadFiles.filter((file) =>
    file.type?.startsWith("video/")
  ).length;
  const otherCount = uploadFiles.length - imageCount - videoCount;
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {uploadFiles.length} archivos en total
                </span>
              </div>
            </div>
            <div className="flex gap-2">
          ...
          <Card className="mb-4">
            <CardContent className="pt-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Selecciona los proveedores donde quieres almacenar tus archivos.
                Ten en cuenta que algunos proveedores solo soportan ciertos
                tipos de archivos.
              </p>
...
const FileUploadItem = ({
  ...
  const { toggleFileExpanded, showProgress } = useBoundStore((s) => s);
  return (
    <Collapsible
      key={file.id}
      open={file.expanded}
      ...
      <div className="p-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {showProgress && file.uploadId && <ProgressBar file={file} />}
          </div>
        </div>
      </div>
...
const ProgressBar = ({ file }: { file: UploadFile }) => {
  useUploadProgress(file.uploadId ?? "", () => {}); // Se suscribe a eventos de este archivo
        ...
        <div className="mt-3 space-y-2 border-t pt-2">
          ...
          <div className="space-y-2">
            {selectedProviders.map((provider) => {
              console.log("file.providerProgress",file.providerProgress)
              ...
              return (
                <div
                  key={`${file.id}-${provider._id}-inline`}
                  className="space-y-1"
                >
                  <div className="flex justify-between text-xs">
